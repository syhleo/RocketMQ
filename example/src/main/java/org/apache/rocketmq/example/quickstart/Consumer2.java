...
public class Consumer2 {
    ...
    public static void main(String[] args) throws InterruptedException, MQClientException, MQClientException {
        ...
        // 再订阅另一个topic
        consumer.subscribe("TopicTestB", "Tag1||Tag3||TagA");
...
//                ,new MyMachineResolver()));
        ...
        consumer.registerMessageListener(new MessageListenerConcurrently() {
            ...
            public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext context) {
                System.out.printf("%s Receive New Messages: %s %n", Thread.currentThread().getName(), msgs.size()); // ConsumeMessageThread_3 Receive New Messages: 1
                //默认情况下，每次都只是一条数据
                for (MessageExt msg : msgs) {
                    System.out.println(Thread.currentThread().getName()+"消费的topic:" + msg.getTopic() + "  【消息】" + new String(msg.getBody()));
                }
                // 标记该消息已经被成功消费

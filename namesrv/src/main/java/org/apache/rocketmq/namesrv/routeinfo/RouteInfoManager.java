...
public class RouteInfoManager {
    ...
    /*
    NameServer是RocketMQ的元数据中心
    类似于Zookeeper之于Kafka。它是整个RocketMQ的大脑，主要负责路由管理、服务注册、服务发现
    NameServer保存的元数据主要包含如下内容: (参见RouteInfoManager)
        topicQueueTable:  Topic队列路由信息，消息发送时根据路由表进行负载均衡
        brokerAddrTable: Broker基础信息，包含broker名称，集群名称，以及主备Broker的地址
        clusterAddrTable: 集群信息，存储着集群中所有的Broker名称
        brokerLiveTable: Broker状态信息，NameServer每次接到心跳时会更新该信息(主要有个最后心跳时间字段)
        filterServerTable: Broker上的filterServer列表，用于类模式消息过滤(在后续的文档中体现)

    https://note.youdao.com/web/#/file/WEB0d126140740e0e535aa77f58d771f6df/note/WEB07dfd5fd613c3189917e5f8e20fb124d/
    ...
    /*
       RouteInfoManager#registerBroker来实现核心注册逻辑
       https://cloud.tencent.com/developer/article/1881875
     */
